(() => {
    ...
    function uploadAreaForm() {
        ...
        if (uploadArea) document.addEventListener("DOMContentLoaded", (function() {
            const fileInput = document.getElementById("fileInput");
            const browseButton = uploadArea.querySelector(".browse-button");
            const uploadMessage = document.getElementById("uploadMessage");
            const fileList = document.getElementById("fileList");
            const allowedTypes = [ "image/jpeg", "image/png", "image/gif", "video/mp4", "application/pdf", "image/vnd.adobe.photoshop", "application/postscript", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation" ];
            ...
            function handleFiles(files) {
                let valid = true;
                for (let file of files) if (!allowedTypes.includes(file.type) || file.size > maxFileSize) {
                    valid = false;
                    break;
                }
                ...
                if (valid) {
                    ...
                    uploadMessage.innerHTML = "✅ File(s) uploaded successfully";
                    Array.from(files).forEach(((file, index) => {
                        ...
                        fileItem.querySelector("button").addEventListener("click", (() => {
                            fileInput.value = "";
                            fileList.innerHTML = "";
                            uploadMessage.innerHTML = "Drag & drop files or";
                            uploadArea.classList.remove("file-loaded", "file-error");
                        }));
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        function updateTotal() {
            let total = 0;
            const prices = orderList.querySelectorAll(".order__row h3");
            prices.forEach((p => {
                const amount = parseFloat(p.textContent.replace("$", ""));
            ...
            orderTotal.textContent = formatPrice(totalWithTax);
        }
        function removeRow(row) {
            const type = row.getAttribute("data-type");
            const id = row.getAttribute("data-id").replace(`${type}-`, "");
            if (type === "service-type" && serviceTypeSelect) {
                const option = serviceTypeSelect.querySelector(`.select__option[data-value="${id}"]`);
                if (option) {
        ...
        function updateServiceTypeSelectHeader() {
            const selectedOptions = serviceTypeSelect.querySelectorAll(".select__option._select-selected");
            const labels = Array.from(selectedOptions).map((opt => opt.textContent.trim()));
            const header = serviceTypeSelect.querySelector(".select__content");
            if (header) header.textContent = labels.length ? labels.join(", ") : "select";
        }
        function addService({value, label, type, price}) {
            const serviceId = `${type}-${value}`;
            const existing = orderList.querySelector(`[data-id="${serviceId}"]`);
            if (existing) return;
        ...
        if (processingSelect) {
            const options = processingSelect.querySelectorAll(".select__option");
            options.forEach((option => {
                option.addEventListener("click", (function() {
                    const value = this.getAttribute("data-value");
                    const price = processingPrices[value] ?? 0;
                    if (!value) return;
                    addOrReplaceService({
                        value,
                        label: "Processing",
        ...
        if (serviceTypeSelect) {
            const observer = new MutationObserver((() => {
                ...
                if (!isOpen) {
                    ...
                    selectedValues.forEach((value => {
                        const label = serviceTypeSelect.querySelector(`.select__option[data-value="${value}"]`).textContent.trim();
                        const price = serviceTypePrices[value] ?? 0;
                        addService({
                            value,
                            label,
                            type: "service-type",
                    ...
                    const existingRows = orderList.querySelectorAll('[data-type="service-type"]');
                    existingRows.forEach((row => {
                        const rowId = row.getAttribute("data-id").replace("service-type-", "");
                        if (!selectedValues.includes(rowId)) removeRow(row);
            ...
            observer.observe(serviceTypeSelect, {
                attributes: true,
                attributeFilter: [ "class" ]
            });
        }
        function addOrReplaceService({value, label, type, price}) {
            const serviceId = `${type}`;
            const existing = orderList.querySelector(`[data-id="${serviceId}"]`);
            if (existing) {
                existing.querySelector("p").textContent = label;
                existing.querySelector("h3").textContent = formatPrice(price);
            } else {
                const row = document.createElement("div");
                row.className = "order__row";
                row.setAttribute("data-type", type);
        ...
        if (translationInput && translationBtn) translationBtn.addEventListener("click", (function(e) {
            e.preventDefault();
            const count = parseInt(translationInput.value.trim());
            ...
            if (existing) {
                existing.querySelector("p").textContent = label;
                existing.querySelector("h3").textContent = formatPrice(totalPrice);
            } else {
                const row = document.createElement("div");
                row.className = "order__row";
                row.setAttribute("data-type", type);
        ...
        if (translationInput) translationInput.addEventListener("input", (() => {
            removeHighlight(translationInput);
        }));
        function focusField(type) {
            if (type === "processing-time" && processingSelect) {
                processingSelect.scrollIntoView({
                    behavior: "smooth",
                    block: "center"
                });
                highlightField(processingSelect);
            }
            if (type === "delivery" && deliverySelect) {
            ...
            if (type === "service-type" && serviceTypeSelect) {
                ...
                highlightField(serviceTypeSelect);
            }
            if (type === "translation" && translationInput) {
                translationInput.scrollIntoView({
                    behavior: "smooth",
                    block: "center"
                });
                highlightField(translationInput);
            }
        }
    }));
})();